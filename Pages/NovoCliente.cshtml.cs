...
namespace SuperOnline.Pages
{
    public class NovoClienteModel : PageModel
    {
        ...
        public NovoClienteModel(SuperOnlineContext context, IEmailSender emailSender,
			...
			if (await TryUpdateModelAsync(cliente, Cliente.GetType(), nameof(Cliente)))
			{
				...
				if (result.Succeeded)
				{
					...
					if (afetados > 0)
					{
						...
						msg.Append("<h1>Super Online :: Confirmação de E-mail</h1>");
						msg.Append($"<p>Por favor, confirme seu e-mail " +
							$"<a href='{HtmlEncoder.Default.Encode(urlConfirmacaoEmail)}'>clicando aqui</a>.</p>");
						msg.Append("<p>Atenciosamente,<br>Equipe de Suporte Super Online</p>");
